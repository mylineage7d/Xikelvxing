 ...
 */
+ (ApiHandler *) sharedInstanceWithKey:(NSString *)projectKey projectID:(NSString *)projectID
{
    NSAssert(projectKey != nil, @"projectKey must not be nil when calling [ApiHandler sharedInstanceWithKey]");
    ...
    @synchronized(self)
    {
        ...
        ApiHandler *handler = [handlers objectForKey:projectKey];
        if (handler == nil)
        {
            handler = [[ApiHandler alloc] init:projectKey projectID:projectID];
            [handlers setObject:handler forKey:projectKey];
...
- (instancetype) init:(NSString*)projectKey projectID:(NSString *)projectID
{
    if (self = [super init])
    {
...
- (BOOL) sendQueuedForDispatch
{
    ...
    @synchronized(_store)
    {
        ...
        while (nil != (apiCall = [_store peekQueue:_projectKey projectID:_projectID]))
        {
            ApiDispatcherResult *result = [_dispatcher dispatch:apiCall projectKey:_projectKey projectID:_projectID];
