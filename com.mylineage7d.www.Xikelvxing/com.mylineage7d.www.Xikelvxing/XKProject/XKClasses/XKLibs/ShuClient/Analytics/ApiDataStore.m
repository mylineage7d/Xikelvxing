...
#define _CALQ_DB_FILENAME @"shu.db"
...
- (BOOL) addToQueue:(AbstractAnalyticsApiCall*)apiCall
{
    @synchronized([ApiDataStore class])
    {
        ...
        {
            const char *insertSql = [[NSString stringWithFormat: @"INSERT INTO %@ (project_id, endpoint, payload) VALUES (? ,?, ?)", _CALQ_DB_TABLE_APIQUEUE] UTF8String];
        ...
        if(sqlite3_bind_text(self->_insertApiStmt, 1, [[apiCall projectID] UTF8String], -1, SQLITE_TRANSIENT) != SQLITE_OK)
        {
            return NO;
        }
 ...
 */
- (QueuedApiCall *) peekQueue:(NSString*)projectKey projectID:(NSString *)projectID
{
    @synchronized([ApiDataStore class])
    {
        if(self->_peekApiStmt == nil)
        {
            const char *peekSql = [[NSString stringWithFormat: @"SELECT id, project_ID, endpoint, payload FROM %@ WHERE project_ID = ? ORDER BY id ASC LIMIT 1", _CALQ_DB_TABLE_APIQUEUE] UTF8String];
        ...
        if(sqlite3_bind_text(self->_peekApiStmt, 1, [projectID UTF8String], -1, SQLITE_TRANSIENT) != SQLITE_OK)
        {
            return nil;
        }
        ...
        if(sqlite3_step(self->_peekApiStmt) == SQLITE_ROW)
        {
            ...
            char *resultProjectID = (char *) sqlite3_column_text(self->_peekApiStmt, 1);
            ...
            return [[QueuedApiCall alloc] initWithId:resultId endpoint:[[NSString alloc] initWithUTF8String:resultEndpoint] payload:[[NSString alloc] initWithUTF8String:resultPayload] projectKey:nil projectID:[[NSString alloc] initWithUTF8String:resultProjectID]];
        }
        else
        {
            // No data!
