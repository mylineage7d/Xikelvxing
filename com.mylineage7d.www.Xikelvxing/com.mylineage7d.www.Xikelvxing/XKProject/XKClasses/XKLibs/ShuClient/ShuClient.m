 ...
 */
+ (ShuClient *) initSharedInstanceWithKey:(NSString *)projectKey projectID:(NSString *)projectID
{
    @synchronized(self)
    {
        if (client == nil)
        {
            client = [[ShuClient alloc] initWithKey:projectKey projectID:projectID];
        }
        else
        {
            if(![projectKey isEqualToString:client->_projectKey])
            {
 ...
 * @param writeKey		The write key to use when communicating with the API.
 */
- (instancetype) initWithKey:(NSString *)projectKey projectID:(NSString *)projectID
{
    return [self initWithKey:projectKey projectID:projectID loadState:YES];
}
 ...
 */
- (instancetype) initWithKey:(NSString *)projectKey projectID:(NSString *)projectID loadState:(BOOL)loadState
{
    NSAssert(projectKey != nil && [projectKey length] >= 32, @"a valid writeKey must be specified");
    if (projectKey == nil || [projectKey length] < 32)
    {
    ...
    if (self = [self init])
    {
        ...
        _apiHandler = [ApiHandler sharedInstanceWithKey:projectKey projectID:projectID];
...
- (void) track:(NSString *)action properties:(NSDictionary *)properties
{
    ...
    if([ShuClient checkPropertyTypes:properties])
    {
        ...
        [self enqueueApiCall:[[ActionApiCall alloc] initWithData:self.actor action:action properties:mergedProperties projectID:self->_projectID projectKey:self->_projectKey]];
...
- (void) setGlobalProperty:(NSString *)property value:(id)value
{
    ...
    if (!([ShuClient checkValueTypes:value]))
    {
...
- (void) identify:(NSString *)actor
{
    @synchronized(self)
    {
        if (![actor isEqualToString:self.actor])
        {
            ...
            if(_hasTracked)
            {
                [self enqueueApiCall:[[TransferApiCall alloc] initWithData:oldActor newActor:self.actor projectKey:self->_projectKey projectID:self->_projectID]];
            }
...
- (void) profile:(NSDictionary *)properties
{
    ...
    [self enqueueApiCall:[[ProfileApiCall alloc] initWithData:self.actor properties:properties projectKey:self->_projectKey projectID:self->_projectID]];
}
...
+ (BOOL)checkPropertyTypes:(NSDictionary *)properties
{
    for (id k in properties)
    {
        ...
        if (!([ShuClient checkValueTypes:properties[k]]))
        {
